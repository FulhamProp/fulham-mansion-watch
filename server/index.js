...
function buildQuery() {
  ...
  return `
    ...
    SELECT ?street ?paon ?saon ?amount ?date ?postcode
    WHERE {
      ?transx ppd:pricePaid ?amount ;
              ppd:transactionDate ?date ;
              ppd:propertyAddress ?addr ;
              ppd:propertyType <http://landregistry.data.gov.uk/def/common/terraced-property> .
      ?addr lrcommon:postcode ?postcode ;
            lrcommon:street ?street .
      OPTIONAL { ?addr lrcommon:paon ?paon }
      OPTIONAL { ?addr lrcommon:saon ?saon }
      FILTER(regex(?postcode, "^${POSTCODE_PREFIX}"))
