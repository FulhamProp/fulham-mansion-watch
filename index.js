...
function buildQuery() {
  ...
  // Land Registry property type URIs (verified against their SPARQL explorer):
  ...
  // Note: postcode stored as e.g. "SW6 1AA" — space included, so regex on "SW6" works.
  ...
  return `
    ...
    SELECT ?street ?amount ?date ?postcode ?propertyType
    WHERE {
      ...
      FILTER(?propertyType IN (
        <http://landregistry.data.gov.uk/def/common/terraced>,
        <http://landregistry.data.gov.uk/def/common/terraced-property>
      ))
    }
    ORDER BY ?street
    LIMIT 5000
